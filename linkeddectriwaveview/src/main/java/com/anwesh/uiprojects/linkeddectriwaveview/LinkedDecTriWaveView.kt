...
class LinkedDecTriWaveView (ctx : Context) : View(ctx) {
    ...
    data class DTWNode(var i : Int, val state : DTWState = DTWState()) {
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            ...
            canvas.drawLine((gap / 2) * sc1, 0f, gap/2, 0f, paint)
            canvas.drawLine(gap/2 + (gap/4) * sc2, -gap/4 * sc2, gap/2 + (gap/4), -gap/4, paint)
            canvas.drawLine(gap/2 + gap/4 + gap /4 * sc3, -gap/4 + gap/4 * sc3, gap, 0f, paint)
            canvas.restore()
